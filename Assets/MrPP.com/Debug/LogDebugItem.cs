	...
	public class LogDebugItem : MonoBehaviour {
        ...
        */
		...
		private static string ToText (LogDebugItem.Item item){
			//;
			return "<color=#"+item.color.r.ToString("x2") + item.color.g.ToString("x2") +item.color.b.ToString("x2")+">" +item.no.ToString() +":"+item.text +"</color>";
			//return ret;
		}
        ...
        public void message()
        {
            ...
            LogDebug.Instance.print(item_.message);
            //Debug.Log(item_.message);
            //LogDebug.Instance.locked () ;
        }
    }
}